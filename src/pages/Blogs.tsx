    ...
    const fetchData = async () => {
      try {
        ...
        if (user) {
          ...
          const ids = new Set(bookmarksData.filter((b: any) => b.contentType === "blog").map((b: any) => b.contentId));
  ...
  return (
    <div className="pt-32 pb-20 px-6 max-w-7xl mx-auto">
      <header className="mb-12">
        <h1 className="text-4xl md:text-5xl font-serif font-bold text-stone-900 mb-4">Insights & Perspectives</h1>
        <p className="text-stone-600 text-lg max-w-2xl">Explore our collection of articles on productivity, mindset, and personal growth.</p>
      </header>
        ...
        {blogs.map((blog) => (
          ...
          >
            <div className="aspect-video overflow-hidden relative group">
              ...
              >
                {bookmarkedIds.has(blog._id) ? <Check className="w-4 h-4" /> : <Bookmark className="w-4 h-4" />}
              </button>
            </div>
            <Link to={`/blog/${blog.slug}`} className="p-6 block">
              <div className="flex flex-wrap gap-2 mb-4">
                {blog.tags.map((tag) => (
                  <span key={tag} className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-stone-500 bg-stone-100 px-2 py-1 rounded-md">
